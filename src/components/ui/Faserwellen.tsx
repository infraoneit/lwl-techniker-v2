...
const BAENDER = [
  { s: 1, a0: 192, a1: 306 },
  { s: 0.925, a0: 196, a1: 296 },
  { s: 0.855, a0: 200, a1: 288 },
  { s: 0.79, a0: 204, a1: 280 },
];
...
export function Faserwellen() {
  ...
  useEffect(() => {
      ...
      BAENDER.forEach((b, band) => {
        for (let i = 0; i < FASERN_PRO_BAND; i++) {
          ...
          fasern.push({
            ...
            alpha: kern ? 0.9 : zufall(0.4, 0.7),
            breite: kern ? 2.6 : zufall(1.1, 1.8),
            kern,
          });
        }
      });
    ...
    const schub = () => {
      const anzahl = Math.round(zufall(6, 12));
    ...
    const zeichnen = (bewegt: boolean) => {
      ...
      // Leuchtsaum entlang jedes Bandes, zwei Lagen für einen weichen Rand
      for (const f of fasern) {
        if (!f.kern) continue;
        const p = fortschritt(f);
        if (p < 0.02) continue;
        spurZiehen(f, 0, p);
        ctx.strokeStyle = `rgba(240,168,0,${(0.03 + 0.04 * atem) * hellFaktor})`;
        ctx.lineWidth = Math.max(70, hoehe * 0.085);
        ctx.stroke();
        ctx.strokeStyle = `rgba(240,200,0,${(0.04 + 0.05 * atem) * hellFaktor})`;
        ctx.lineWidth = Math.max(26, hoehe * 0.03);
        ctx.stroke();
      }

      // Fasern mit Verlauf von Gelb zu Orange
      for (const f of fasern) {
        const p = fortschritt(f);
        if (p < 0.01) continue;
        setzePunkt(f, 0);
        ...
        const alpha = Math.min(1, f.alpha * hellFaktor * (0.8 + 0.2 * atem));
        verlauf.addColorStop(0, `rgba(${gelb},${alpha})`);
        verlauf.addColorStop(1, `rgba(${orange},${alpha})`);
        spurZiehen(f, 0, p);
      ...
      for (const pl of pulse) {
        ...
        schweif.addColorStop(1, `rgba(${schweifKopf},${0.6 * pl.staerke})`);
        spurZiehen(f, start, kopf);
        ctx.strokeStyle = schweif;
        ctx.lineWidth = 2;
        ctx.stroke();

        const radius = 9 + 7 * pl.staerke;
        const glut = ctx.createRadialGradient(punkt.x, punkt.y, 0, punkt.x, punkt.y, radius);
        glut.addColorStop(0, `rgba(${glutKern},${0.9 * pl.staerke})`);
        glut.addColorStop(0.35, `rgba(240,200,0,${0.5 * pl.staerke})`);
      ...
      for (const b of blitze) {
        if (bewegt) b.leben -= 0.028;
        if (b.leben <= 0) continue;
        setzePunkt(fasern[b.faser], 1);
        const radius = 16 + 30 * (1 - b.leben);
        const glut = ctx.createRadialGradient(punkt.x, punkt.y, 0, punkt.x, punkt.y, radius);
        glut.addColorStop(0, `rgba(${glutKern},${0.75 * b.leben * b.staerke})`);
        glut.addColorStop(0.4, `rgba(240,190,0,${0.35 * b.leben * b.staerke})`);
        glut.addColorStop(1, 'rgba(240,168,0,0)');
        ctx.fillStyle = glut;
        ctx.beginPath();
        ctx.arc(punkt.x, punkt.y, radius, 0, Math.PI * 2);
